...
5.  **Implement a Client (Optional)**: Using the `APIClient` from yesterday, write a few functions that _would_ interact with your designed API, even though the server doesn't exist. This helps you think about the design from a user's perspective.
